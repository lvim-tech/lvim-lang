-- lvim-lang.providers.java.tasks: build / run / test through the project's build tool.
-- Gradle and Maven are the two Java build systems; `buildtool.detect` picks the right one at the
-- resolved root and `buildtool.base` the right binary (the `./gradlew` / `./mvnw` wrapper when the
-- project ships one, else the system `gradle` / `mvn`). build / run / test are fire-and-collect, so
-- they go through core.runner → lvim-tasks (its panel / history / dock). javac / gradle / maven emit
-- `file:line: error: message`, routed to the quickfix by the built-in `generic` matcher. Extra
-- command-line args are appended (e.g. `:LvimLang build --info`, `:LvimLang test --tests Foo`).
--
---@module "lvim-lang.providers.java.tasks"
