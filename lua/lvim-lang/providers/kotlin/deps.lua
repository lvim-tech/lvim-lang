-- lvim-lang.providers.kotlin.deps: dependency inspection through the project's build tool.
-- Gradle and Maven declare dependencies in their build scripts (`build.gradle.kts` / `build.gradle`
-- / `pom.xml`), so ADDING / REMOVING a dependency means editing those files by hand — there is no
-- clean, non-destructive CLI verb for it (unlike cargo / npm), and inventing one that rewrites a
-- user's build script would be a kludge. This module therefore exposes the SAFE, read-or-resolve
-- operations: `tree` (the dependency graph), `refresh` (re-resolve, ignoring caches) and `install`
-- (build / install artifacts). Runs land in the lvim-tasks panel (Dependencies group); core.runner
-- runs `:checktime` on exit so an edited lock/metadata reloads in open buffers.
--
---@module "lvim-lang.providers.kotlin.deps"
...
for _, action in ipairs(SUBS) do
    M.templates[#M.templates + 1] = {
        name = "kotlin deps " .. action,
        desc = "Kotlin dependencies: " .. action .. " (auto-detected build tool)",
        group = "Dependencies",
