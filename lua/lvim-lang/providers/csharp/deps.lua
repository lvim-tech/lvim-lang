-- lvim-lang.providers.csharp.deps: NuGet dependency commands, run through lvim-tasks.
...
-- cwd (the project/solution root). core.runner runs `:checktime` on exit, so an edited `.csproj` /
-- lockfile reloads in open buffers. The arg-less subcommands (`restore`, `list`) also register as
-- lvim-tasks templates.
--
---@module "lvim-lang.providers.csharp.deps"
...
--- Resolve the .NET project/solution root for the current buffer (nearest `.sln`/`.csproj`, else
--- `.git`, else the file's directory, else cwd).
---@return string
...
--- Run `dotnet <argv…>` for a root through lvim-tasks (Dependencies group, `typescript` C# matcher).
---@param root string
---@param argv string[]
---@param name string
---@return nil
local function run_dotnet(root, argv, name)
    ...
    runner.run("csharp", { name = name, cmd = cmd, cwd = root, group = "Dependencies", matcher = "typescript" })
end

-- lvim-tasks templates (via the provider's `tasks` field): the arg-less subcommands, each applying
-- only in a .NET project (a `.sln`/`.csproj` at or above the resolved root).
---@type table[]
M.templates = {}
