-- lvim-lang.providers.fsharp.toolchain: the F# / .NET toolchain spec.
...
-- PATH. The language server (`fsautocomplete`), the `fantomas` formatter and the `netcoredbg`
-- debugger are mason-registry binaries: an explicit path → the resolved mason bin directory (where
-- the installer drops them) → PATH. Detection only — nothing is installed here (missing tools come
-- from the mason registry via the installer / core.ensure).
--
---@module "lvim-lang.providers.fsharp.toolchain"

local config = require("lvim-lang.config")

--- The fsharp config block (seeded by the provider's setup defaults).
---@return table
local function opts()
...
return {
    tools = {
        ...
        fsautocomplete = {
            { kind = "path", value = explicit("fsautocomplete_path") },
            { kind = "path", value = in_mason("fsautocomplete") },
            { kind = "which", value = "fsautocomplete" },
        },
        fantomas = {
            { kind = "path", value = explicit("fantomas_path") },
            { kind = "path", value = in_mason("fantomas") },
            { kind = "which", value = "fantomas" },
        },
        netcoredbg = {
    ...
    --- `<bin> --version` — first NON-EMPTY line, trimmed. dotnet / fsautocomplete / fantomas /
    --- netcoredbg all report their version with `--version`.
    ---@param bin string
    ---@return string|nil
